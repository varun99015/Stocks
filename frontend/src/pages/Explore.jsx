  ...
  useEffect(() => {
    const fetchMarketData = async () => {
      try {
        ...
        const response = await axios.get('https://stocks-backend-fdcd.onrender.com/api/stocks/overview');
        
        // Convert the API's array into a map for fast lookups: { SPCE: { price: ..., change: ... }}
        const dataMap = response.data.reduce((acc, stock) => {
          acc[stock.symbol] = stock;
          return acc;
        }, {});

        // Build the final categories array by merging our structure with the live data
        const liveCategories = Object.entries(CATEGORY_STRUCTURE).map(([name, data]) => {
          return {
            ...
            type: data.type,
            stocks: data.symbols
              .map(symbol => dataMap[symbol]) // Look up live data for each symbol
              .filter(Boolean) // Filter out any stocks that might not have data from the API yet
          }
        });
